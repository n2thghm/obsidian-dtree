import { App, ButtonComponent, SuggestModal, setIcon, getIconIds } from "obsidian";

export const dendronActivityBarName = "dendron-activity-bar";
...
export class IconSuggestModal extends SuggestModal<string> {
  ...
  constructor(app: App, buttonEl: ButtonComponent, onSelect: (iconId: string | null) => void) {
    super(app);
    this.button = buttonEl
    this.onSelectCallback = onSelect
  }

  // Fetch all icons available in Obsidian
  ...
  // Show icons in modal result container
  ...
  onChooseSuggestion(iconId: string, evt: MouseEvent | KeyboardEvent) {
    this.onSelectCallback(iconId)
  }

  // Resize and reposition the modal beneath the caller button
  onOpen(): void {
    super.onOpen();

    const buttonRect = this.button.buttonEl.getBoundingClientRect();
...
export function attachIconMenu (buttonEl: ButtonComponent, onSelect: (icon: string | null) => void) {
  const modal = new IconSuggestModal(this.app, buttonEl, onSelect)
